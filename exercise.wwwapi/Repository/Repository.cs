...
namespace exercise.wwwapi.Repository
{
    public class Repository : IRepository
    {
        ...
        public async Task<IEnumerable<Student>> GetStudentsByCourse(int courseId, PreloadPolicy preloadPolicy = PreloadPolicy.DoNotPreloadRelations)
        {
            if (preloadPolicy == PreloadPolicy.PreloadRelations)
            {
                return await _db.Students.Where(s => s.CourseId == courseId).Include(s => s.Course).ToListAsync();
            }
            return await _db.Students.Where(s => s.CourseId == courseId).ToListAsync();
        }

        public async Task<IEnumerable<Course>> GetCoursesByStudent(int studentId, PreloadPolicy preloadPolicy = PreloadPolicy.DoNotPreloadRelations)
        {
            if (preloadPolicy == PreloadPolicy.PreloadRelations)
            {
                return await _db.Courses.Where(c => c.Students.Any(s => s.Id == studentId)).Include(c => c.Students).ToListAsync();
            }
